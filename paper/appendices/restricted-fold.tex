...
\begin{proof}
The endpoint cases $h=0,r$ are immediate, so assume $0<h<r$.  Choose a proof-local identification $S\simeq\mathbf F_p$; the final statement is invariant under this choice.
...
\tag{RF3}
\]
To see this, write $R=r-h$ and $m=qh+s$ with $0\le s<h$.  Take a nondecreasing sequence $\lambda_i\in[0,R]$ with $h-s$ copies of $q$ and $s$ copies of $q+1$, and put $k_i=i-1+\lambda_i$.

Apply this with $m=M$.  In the highest-degree part of \eqref{eq:rf-polynomial}, equation \eqref{eq:rf-vandermonde} gives a nonzero coefficient at $x_1^{k_1}\cdots x_h^{k_h}$.  It is nonzero modulo $p$ because $M\le p-1$ and $0\le k_i\le r-1\le p-1$.  The coefficient detector therefore contradicts the vanishing of $P$ on $A^h$.
