...
We record the additive-combinatorial input used by the local response theorem.  The proof is the standard polynomial-method argument, included to keep the arithmetic dependency explicit.  For a finite set $A$, write $\operatorname{FinSub}_h(A)$ for the set of its $h$-element subsets, with ordinary extensional equality.
