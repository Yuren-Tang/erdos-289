...
Discard at most one point of $X$ and let $\Omega$ be the finite set of perfect
matchings of what remains.  Every unordered pair occurs in the same number of
matchings, so Lemma~\ref{lem:difference-energy-shell} gives
\[
\overline S_t
...
If $N^5/p^4\gg\log p$, finite subadditivity over the $p-1$ nonzero $t$ shows
that there is one matching $M_*$ for which
\[
S_t(M_*)\gg\frac{N^3}{p^2}
\qquad(t\ne0)
\tag{AC4}\label{eq:simultaneous-energy}
\]
simultaneously.
...
$e^{-c_4u}$ gives
\[
\frac1{|C|}\sum_{e\in C}e^{-c_4w_t(e)}
\le
\exp\!\left(-c_5\frac{\sum_{e\in C}w_t(e)}{|C|}\right).
\]
...
\frac{|\chi_t(\Phi_A)|}{2^M}
\le
\exp\!\left(-c_6\frac{N^2M}{p^2}\right).
\tag{AC5}\label{eq:chunk-character}
\]

If
\[
M\ge C\frac{p^2}{N^2}\log p
\tag{AC6}\label{eq:atomic-threshold}
\]
for a sufficiently large absolute $C$, summing \eqref{eq:chunk-character} over
$t\ne0$ shows that the finite arithmetic average of
\[
\frac1{2^M}\sum_{t\ne0}|\chi_t(\Phi_A)|
\]
is $<1$.  Hence some exact-$M$ selection satisfies
\[
\sum_{t\ne0}|\chi_t(\Phi_A)|<2^M.
\tag{AC7}\label{eq:augmentation-dominance}
\]
...
Let $X\subset\mathbf F_p$, $|X|=N$.  If
\[
N^5/p^4\gg\log p,
\]
...
The matching, chunks and characters have no consumer after this theorem.
