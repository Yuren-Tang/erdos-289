...
This appendix has a different status from Appendices~\ref{app:prime-supply} and~\ref{app:restricted-fold}.  It proves no asymptotic theorem: it supplies one finite starting configuration from which the uniform bridge construction of Section~\ref{sec:wide-start} takes over.  The integers below form one explicit witness; no minimality, uniqueness, or arithmetic significance is asserted.
...
\tag{PS2}
\end{equation}
Direct comparison shows that the corresponding component envelopes are compatible with the five displayed configurations and with one another whenever they occur simultaneously.  The list is part of this particular finite witness, not a canonical sequence.

\subsection*{Why the numerical initialization disappears}

Suppose a previous prime-power jump has upper least-common-multiple modulus $D$, so every base in that stage is at most $D$ and every component envelope ends by $D+2$.  At the next nontrivial jump the lower modulus is the same $D$, the quotient prime is $p$, and the smallest new base is
\[
a_k=\frac{pD}{k},
\]
...
a_k-D\ge D/2\ge1260.
\]
The new component envelopes therefore begin far beyond both the finite seed and the preceding bridge stage, and the same estimate persists inductively as the lower moduli increase.  Thus the finite list \eqref{eq:prefix-boundary-bases} really is beyond the uniform-separation threshold.

The reciprocal-mass estimate is likewise uniform.  The largest initial reciprocal value is $5/6$.  For the four finite bridge bases $12,20,30,60$, the crude envelope bound $<3/a$ gives
