...
This appendix proves the ordinary analytic input used in Sections~\ref{sec:signed-inverse} and~\ref{sec:scalar-finality}: one fixed-ratio prime band containing $\asymp X/\log X$ primes.  The multiplier is taken canonically to be the least admissible integer, so every later cutoff remains on the natural-number scale.
...
\begin{equation}\label{eq:chebyshev-linear}
...
The set
\[
\{m\in\N:m\ge2,\ C_\theta<c_\theta m\}
\]
is nonempty.  Let $\Lambda$ be its least element.  Then
\[
