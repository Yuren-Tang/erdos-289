...
Choose once $a<c_1<C_1<A$, where $c_1,C_1$ are the current-window constants in
\eqref{eq:tail-current-window}.  Then every Tail current window $I_Q$ lies
inside $I_Q(a,A)$, so \eqref{eq:uniform-source-closure} implies exactly the
hypothesis of Lemma~\ref{lem:sparse-survival}.  Therefore there are fixed
$c_0>0$ and $B_0$ such that for every $B\ge B_0$ the Prefix $P_B$ is
$c_0$-transparent at its own source $E_B$, both for all currents beyond $B$
and for every boundary-predecessor row.

Combining \eqref{eq:prefix-literal-coset},
\eqref{eq:source-closed-slack}, and this transparency conclusion proves the
finite-prefix input used in Section~\ref{sec:affine-provider}.  The explicit
root identities have now reached their last consumer.
