...
There is $c>0$ such that for all sufficiently large $Y$ the new prime currents
in
\[
Y<p\le\Lambda Y
\]
...
\gg Y/\log Y.
\]
Widths add under Minkowski addition, which proves the claim.
\end{proof}

We next bound the lower endpoint introduced by all new currents up to the next
endpoint $\Lambda Y$.
...
\tag{G6}
\]
The deliberately crude logarithmic factor absorbs the finite sum over relevant
exponents.

Combining (G4)--(G6), the entire new lower displacement satisfies
\[
...
\begin{proof}
By Lemma~\ref{lem:prime-band-width}, the prime width already present by the
next endpoint is
\[
\Omega\!\left(\frac{Y^2}{(\log Y)^2}\right),
\]
while (G7) is only $O(Y\log Y)$.  Since
\[
Y\log Y=o\!\left(\frac{Y^2}{(\log Y)^2}\right),
\]
the width of the preceding interval eventually exceeds the displacement to the
next lower endpoint.  Thus successive intervals overlap or are adjacent.
Their lower endpoints tend to infinity, because every new nontrivial current
contributes positive minimum grade.  Iterating the fixed-ratio endpoint map
therefore yields a final ray.
\end{proof}
...
The proof of BinaryTail therefore separates cleanly into two independent
consumer statements:
\[
