...
There exist $\eta,\sigma>0$ such that, for every integer-valued $L$ satisfying $L(B)=o(B/\log B)$, there is $B_0(L)$ with the following property.  For every $B\ge B_0(L)$ there are a finite set $X_B$ and a covering labelled relation
\[
R_B:X_B\rightsquigarrow \operatorname{Tor}_{E_B}(\gamma_B)\times I_B,
\]
