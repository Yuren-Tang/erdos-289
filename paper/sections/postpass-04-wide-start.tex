...
Fix the finite initialization from Lemma~\ref{lem:finite-prefix-initialization} and compose it with one bridge at every subsequent prime-power jump up to a cutoff $B$.  On residues, finite induction gives a full affine $E_B$-fibre.  Since every bridge adds the same number of components to every branch, the difference between the largest and smallest component counts remains equal to the finite initial spread.  Thus there is a constant $G_0$ and integers $m_B\le M_B$ such that every branch of the resulting affine cover has component count in
\[
[m_B,M_B],
\qquad
M_B-m_B=G_0.
...
\]
Apply Lemma~\ref{lem:neutral-cube} to the bounded-spread affine cover using $G_0+L(B)$ neutral coordinates and arbitrarily small total reciprocal mass.  If a branch has component count $r\in[m_B,M_B]$ and $0\le j\le L(B)$, the required Hamming weight is
\[
(M_B-r)+j.
\tag{4.10}\label{eq:hamming-match}
\]
The construction is literally the pullback
\[
\begin{CD}
...
\]
where $\mathcal B_B$ denotes the branch set of the bounded-spread affine cover.  The right vertical map is surjective, so every affine branch occurs with every requested component count in one common interval of width $L(B)$, without a chosen Hamming representative.
