...
There exist $\eta,\sigma>0$ such that, for every integer-valued $L$ satisfying $L(B)=o(B/\log B)$, there is $B_0(L)$ with the following property.  For every $B\ge B_0(L)$ there is a finite covering labelled relation with target
\[
\operatorname{Tor}_{E_B}(\gamma_B)\times I_B,
\]
where $I_B$ is the interval in \eqref{eq:common-count-interval} and every interval label has reciprocal value below $2-\eta$.  Moreover every later prime-power family satisfies
\[
|\mathcal A_Q(B)|\ge\sigma|\mathcal A_Q|.
\]
\end{proposition}
