...
k_++k_-=b.
\]
Since $p\nmid b$, the two coefficients cannot both be divisible by $p$.  Define a sign deterministically: if $e=1$, use the sign with larger coefficient; if $e\ge2$, use $+$ unless $p\mid k_+$, and use $-$ otherwise.  Let $k=k(Q,b)$ be the coefficient determined by this rule, and define
...
For the canonical quantity \eqref{eq:canonical-role-mass}, a prime family has $O(p/\log p)$ intervals, each of reciprocal mass $O(p^{-2})$, so
\[
...
Define the canonical positive constants
\begin{equation}\label{eq:canonical-density-support-constants}
\kappa=\frac{\kappa_\infty}{2},
\qquad
c=\frac{c_\infty}{2}.
\end{equation}
By the definitions of the lower limits, for every sufficiently large prime power $Q$ one has
...
\rho_P=\rho_P(\Delta_*),
\end{equation}
with the canonical functions of \eqref{eq:haxell-canonical-parameters}.  These constants will be used in the finite compatibility step.
