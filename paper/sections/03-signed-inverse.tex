...
\end{equation}
Fix $(\ell_0,\pm)$.  If $p\mid\ell_0$, the congruence has no solution because its right-hand side is a unit modulo $p$.  Otherwise it reduces to a unit square congruence modulo $p^e$, which has at most two roots for odd $p$ and at most four roots for $p=2$.  There are only $2(B_{\mathrm{car}}-1)$ choices of $(\ell_0,\pm)$, proving the first assertion.
