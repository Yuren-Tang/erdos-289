...
At the jump $n=p^e$, one has $E_{n-1}=H_D$ and $E_n=H_{pD}$, and the new simple quotient is $E_n/E_{n-1}$.  Its relation to the ambient residue group is the pullback
\begin{equation}\label{eq:bridge-simple-pullback}
\begin{CD}
E_n @>>> A\\
@V{q}VV @VV{q_{E_{n-1}}}V\\
E_n/E_{n-1} @>>> A/E_{n-1},
\end{CD}
\end{equation}
where the bottom arrow is the natural inclusion.

Let $\mathscr B_n$ be the finite family of bridge labels obtained by choosing one alternative in \eqref{eq:bridge-switch} at every $r\in R_p$.  Fix a reference branch $B_n^\circ\in\mathscr B_n$ and put
\[
\beta_n=\overline W(B_n^\circ).
\]
The preceding subset-sum calculation says that the difference map
\begin{equation}\label{eq:bridge-difference-surjection}
\Delta_n:\mathscr B_n\longrightarrow E_n/E_{n-1},
\qquad
B\longmapsto[\overline W(B)-\beta_n]
\end{equation}
is surjective.

\begin{lemma}[Affine bridge extension]\label{lem:affine-bridge-extension}
Let $\alpha\in A/E_{n-1}$, and let $\mathscr S\subseteq\mathcal C$ be a finite family whose residue map covers $\operatorname{Tor}_{E_{n-1}}(\alpha)$.  Suppose every $S\in\mathscr S$ is jointly compatible with every $B\in\mathscr B_n$.  If $\alpha^+\in A/E_n$ is
\[
\alpha^+=\pi_{n-1,n}(\alpha)+q_{E_n}(\beta_n),
\]
where $\pi_{n-1,n}:A/E_{n-1}\to A/E_n$ is the quotient map, then the unions
\[
\{S\cup B:S\in\mathscr S,\ B\in\mathscr B_n\}
\]
cover the full affine fibre $\operatorname{Tor}_{E_n}(\alpha^+)$.  Every bridge branch adds exactly $k$ connected components, so the component-count spread of $\mathscr S$ is unchanged.
\end{lemma}

\begin{proof}
Every bridge residue belongs to $\beta_n+E_n$ by \eqref{eq:bridge-difference-surjection}, so every displayed union lies in the asserted target fibre.  Conversely fix
\[
y\in\operatorname{Tor}_{E_n}(\alpha^+)
\]
and choose $x_0\in\operatorname{Tor}_{E_{n-1}}(\alpha)$.  Then
\[
y-x_0-\beta_n\in E_n.
\]
By the surjectivity of \eqref{eq:bridge-difference-surjection}, choose $B\in\mathscr B_n$ whose difference class agrees with the class of $y-x_0-\beta_n$ in $E_n/E_{n-1}$.  Hence
\[
y-\overline W(B)-x_0\in E_{n-1},
\]
so $y-\overline W(B)$ lies in $\operatorname{Tor}_{E_{n-1}}(\alpha)$.  Choose $S\in\mathscr S$ with
\[
\overline W(S)=y-\overline W(B).
\]
Joint compatibility gives $S\cup B\in\mathcal C$, and its residue is $y$.  Since every $B\in\mathscr B_n$ has exactly $k$ components and is separated from $S$, one has
\[
\nu(S\cup B)=\nu(S)+k,
\]
which preserves the spread.
\end{proof}

\subsection{Bounded-spread affine cover}

Fix the finite initialization from Lemma~\ref{lem:finite-prefix-initialization} and compose it with one bridge at every subsequent prime-power jump up to a cutoff $B$.  Applying Lemma~\ref{lem:affine-bridge-extension} at each jump gives, by finite induction, a full affine $E_B$-fibre.  Let $\mathcal B_B\subset\mathcal C$ be the finite set of interval labels occurring in this affine cover.  Since every bridge adds the same number of components to every label, the difference between the largest and smallest component counts remains equal to the finite initial spread.  Thus there is a constant $G_0$ and integers $m_B\le M_B$ such that
...
\right\}.
\end{equation}
Fix $L\in\mathfrak L$.  Equivalently,
...
Let $\mathscr P_B$ be the finite pool of connected interval components that occur in at least one label of the prepared initial cover.  Its members are precisely drawn from the fixed seed, the bridge alternatives through $B$, and the neutral-coordinate alternatives.  The neutral family contributes $O(G_0+L(B))=o(B/\log B)$ bounded-size exclusion envelopes.  Since $Q/\log Q$ is eventually increasing, this is uniformly $o(Q/\log Q)$ for every $Q\ge B$.  Together with \eqref{eq:bridge-footprint}, the $O(1)$ seed contribution and \eqref{eq:family-size}, this gives constants $\sigma,\eta>0$, independent of $L$, such that the following holds.  Let
\begin{equation}\label{eq:compatible-subfamily}
\mathcal A_Q(B)
=
\{I\in\mathcal A_Q:I\text{ is compatible with every }J\in\mathscr P_B\}.
\end{equation}
Equivalently, $\mathcal A_Q(B)$ consists of the members of $\mathcal A_Q$ compatible with every label of the initial cover at $B$.  After increasing the onset cutoff if necessary,
...
There exist $\eta,\sigma>0$ such that, for every $L\in\mathfrak L$, there is $B_0(L)$ with the following property.  For every $B\ge B_0(L)$ there is a covering observable labelled relation
\[
