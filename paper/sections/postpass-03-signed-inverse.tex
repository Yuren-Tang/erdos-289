...
\item There are nonnegative numbers $\omega_Q$ such that every local labelled relation formed from $\mathcal A_Q$ below has reciprocal mass at most $\omega_Q$, and
...
\]
Summing over the fixed geometric prime bands from Proposition~\ref{prop:prime-supply-main} converges.  For $Q=p^e$, $e\ge2$, a local labelled relation uses at most $p$ intervals, each of mass $O(\log Q/Q^2)$, so
\[
\omega_{p^e}
