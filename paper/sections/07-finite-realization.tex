...
\end{equation}
The dependency order used below is
\begin{equation}\label{eq:realization-dependency}
\begin{CD}
(B_{\mathrm{mass}},B_{\mathrm{pack}}) @>>> B @>>> X\\
@. @. @VVV\\
@. @. P_X\\
@. @. @VVV\\
@. @. \{\mathscr L_X(Q):B<Q\le X\}\\
@. @. @VVV\\
@. @. R_X.
\end{CD}
\end{equation}
Here $B$ comes from scalar terminality, a finite $X$ is fixed before Haxell is invoked, $P_X$ is then chosen by Haxell, the uniform local theorem is replayed on the surviving rows to form the $\mathscr L_X(Q)$, and joint compatibility finally strictifies their composite to $R_X$.
