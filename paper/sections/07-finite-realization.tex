...
This section lifts the scalar terminal plan back to the physical labelled layer.  Only after the finite cutoff $X$ has been fixed do we use Haxell to select mutually compatible surviving rows.  The uniform simple-quotient supports are then lifted to covering affine translations by Corollary~\ref{cor:graded-affine-translation}, and joint compatibility strictifies their labelled composite.
...
\end{CD}
\end{equation}
Thus Corollary~\ref{cor:graded-affine-translation} is precisely the lifting cell from a simple-quotient count support to a covering observable translation between the corresponding affine torsor fibres.
...
\[
T_0=\operatorname{Tor}_{E_B}(\gamma_B)\times I_B,
\]
and for $1\le j\le m$ let
\[
\gamma_j=\pi_{B,Q_j}(\gamma_B),
\qquad
T_j=\operatorname{Tor}_{E_{Q_j}}(\gamma_j)\times I_{Q_j}.
\]
