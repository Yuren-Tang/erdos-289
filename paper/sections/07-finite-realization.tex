...
\end{equation}
The dependency order used below is the ordinary implication flow
...
\{\mathscr L_X(Q):B<Q\le X\}\longrightarrow R_X.
\end{aligned}
\end{equation}
Here $B$ comes from Theorem~\ref{thm:scalar-terminality}, a finite $X$ is fixed before Haxell is invoked, $P_X$ is then chosen by Haxell, the uniform local theorem is replayed on the surviving rows to form the $\mathscr L_X(Q)$, and joint compatibility finally strictifies their composite to $R_X$.
...
The local theorem concerns $S_Q=F_Q/F_{<Q}$, while the global induction concerns affine fibres in $A=\Q/\Z$.  The quotient map sits in the short exact sequence
\begin{equation}\label{eq:simple-extension}
\begin{CD}
0 @>>> S_Q @>>> A/F_{<Q} @>>> A/F_Q @>>> 0.
\end{CD}
\end{equation}
Thus the fibre of $A/F_{<Q}\to A/F_Q$ is canonically an $S_Q$-torsor.  The same fact is expressed by the pullback square
