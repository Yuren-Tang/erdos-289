...
\begin{proof}
The empty configuration is compatible with every label, so $1_X$ is an identity.  For three composable labelled relations, either parenthesization selects exactly the same triples of edges whose three labels have jointly compatible literal union, and assigns that union as the composite label.  Since literal union is associative, the two composites coincide.  Monotonicity under inclusion is immediate from the definition.
\end{proof}
...
\begin{CD}
& P_n &\\
@V{\widetilde\ell}VV @VV{\ell}V\\
D_n @>{\iota}>> \mathcal C^n .
\end{CD}
\end{equation}
