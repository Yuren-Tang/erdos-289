...
Let $\mathcal O$ be a finite family of physical obstacles, let $\varepsilon\in\Q_{>0}$, and let $L\ge0$.  There exist $L$ mutually compatible copies of \eqref{eq:neutral-identity}, all compatible with every obstacle in $\mathcal O$, such that the sum of their common reciprocal costs is less than $\varepsilon$.
...
0,1,\ldots,L.
\]
Moreover, for each coordinate the union of the complete component-envelope families of its two alternatives has cardinality at most $5$.
\end{lemma}

\begin{proof}
Construct the coordinates inductively.  At stage $i$, enlarge the finite obstacle by both alternatives from all preceding coordinates.  Choose $a$ so large that both new alternatives lie beyond this finite support by more than one vertex and have common reciprocal value below $\varepsilon/(L+1)$.  This gives compatibility with all previous choices and total cost $<\varepsilon$.  The two alternatives in one coordinate have respectively two and three complete component envelopes, so their combined envelope complexity is at most $5$.  The Hamming-weight map $\{0,1\}^L\twoheadrightarrow\{0,\ldots,L\}$ gives the grade shifts without choosing a distinguished selector in any fibre.
\end{proof}

Whenever a later argument is stated with a positive real resource tolerance, we first choose a positive rational tolerance below it and apply the lemma.  Thus no change of tolerance field is hidden at the neutral-cube interface.
