...
This appendix proves the ordinary analytic input used in Sections~\ref{sec:signed-inverse} and~\ref{sec:scalar-finality}: one fixed-ratio prime band containing $\asymp X/\log X$ primes.  We take the multiplier to be the least admissible integer, so every later cutoff remains on the natural-number scale.
