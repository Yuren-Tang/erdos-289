...
/-! The following bounded lcm is only an internal detector for nonvanishing in
the intrinsic simple fibre; it is not part of the filtration interface. -/

private def lowerStageExponent (Q : ℕ) : ℕ :=
  (Finset.Icc 1 (Q - 1)).lcm id

private theorem lowerStageExponent_ne_zero (Q : ℕ) :
    lowerStageExponent Q ≠ 0 := by
  simp [lowerStageExponent, Finset.lcm_eq_zero_iff]

private theorem lowerPrimePowerStage_le_annihilatorStage_lowerStageExponent
    (Q : ℕ) :
    lowerPrimePowerStage Q ≤ annihilatorStage (lowerStageExponent Q) := by
  unfold lowerPrimePowerStage
...
private theorem prime_pow_not_dvd_lowerStageExponent
  ...
  have hfacLt : (lowerStageExponent (p ^ e)).factorization p < e := by
    rw [lowerStageExponent, Finset.factorization_lcm]
