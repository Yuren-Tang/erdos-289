...
/--
The generic finite torsor seed is a core stage: for every `D ≥ 1`, every
physical constraint, and *every* barrier slack `s < 1`, there is a `CoreStage`
whose subgroup is the cyclic group generated by `[1/D]` and whose slack is `s`.

The slack is a parameter because the seed's mass is `β + j/D` with `β`
arbitrarily small and `j/D < 1`: the barrier `2 - sup W` can be brought
arbitrarily close to `1` and is not a distinguished number.
-/
theorem exists_coreStage (c : PhysicalConstraint) {D : ℕ} (hD : 0 < D)
  ...
  obtain ⟨L⟩ := exists_ladder c hqpos (D - 1) (ε := 1 - s) (by linarith)
  refine ⟨{
...
theorem smallBlockSaturation_of_tailInterface
  ...
  obtain ⟨B, hsub, hslack⟩ := exists_coreStage originalConstraint hD hs0 hs1
